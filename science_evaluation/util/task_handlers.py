...
class AIMETaskHandler(MathTaskHandler):
    ...
    def generate_prompt(prompt, model):
        if "Sky-T1" in MODEL_TO_NAME.get(model, 'fuse'):
...
class GPQADiamondTaskHandler(TaskHandler):
    ...
    def __init__(self, data_dir="./data/gpqa"):
    ...
    def load_and_filter_dataset(self, start, end, split="train", source=None, filter_difficulty=False, args=None):
        file_path = os.path.join(self.data_dir, f"gpqa_diamond.csv")
        if not os.path.exists(file_path):
            raise FileNotFoundError(f"Dataset file not found at {file_path}. Please download it manually.")
