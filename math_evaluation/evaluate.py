...
def evaluate(data_name, prompt_type, samples: list=None, file_path: str=None, max_num_samples=None, execute=False):
    ...
    row_max = np.array(score_mat).max(axis=1)
    row_score = float(np.round(row_max.mean() * 100, decimals=1))

    average_acc = float(np.round(np.mean(col_means) * 100, decimals=1))
    ...
    result_json = {
        ...
        "avg_acc": average_acc,          # 所有列的平均值
        "acc": mean_score[0],
        "max_acc": max(mean_score),
        "min_acc": min(mean_score),
        f"pass@{len(mean_score)}": row_score
    }
